...
//	static function filter_to_html_color($filter,$count,$filter_selected,$filters_shown,$url){
	...
	static function writeFormEnd() {
		if(chpconf::option('type')==1){
			...
			echo '</form>';
                                            echo "<a href='#'>Отчистить</a>";

		}
	}
	
	static public function writePriceForm($category_id, $form,$price_selected,$url){
		$t='';
                $oldurl = $url;
                if($category_id){
                    $splitUrl = split('category_id=', $url);
                    $splitUrl[0] = $splitUrl[0] . 'category_id=' . $category_id;
...
//                var_dump($url);die;
		if(chpconf::option('type')==0){
			...
			if(chpconf::option('collapsehead')){
				...
				if ($price_selected) {
                                    $psevdoUrl = '';
                                    $t.='<a href="' . $url . '" class="chp-pa">' . chpconf::option('clear') . '</a>';
                                    $t.='</h2>';
