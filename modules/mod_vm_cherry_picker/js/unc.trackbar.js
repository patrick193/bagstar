...
var chp_trackbar=new Class({
	...
	//	}else{
		...
		this.inputLeft=document.id(this.options.form[this.options.inputLeft]);
		if(!mode){
			this.inputRight=document.id(this.options.form[this.options.inputRight]);
	...
	start:function(event){
		...
		//top=this.elem.getTop()-window.getScrollTop();
		top=getY(this.elem)-window.getScrollTop();
		//console.log(this.elem.getTop(), getY(this.elem), this.elem.offsetTop, window.getScrollTop());
		// with this move we set fixed position to the element in order to get it out of document flow for best fps
