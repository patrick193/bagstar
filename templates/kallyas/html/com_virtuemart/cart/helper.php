...
class CartHelper {
	...
	public function getPrices() {
		...
		foreach($cartData["DBTaxRulesBill"] as $id=>$item) {
		    if(isset($this->cart->pricesUnformatted[$id.'Diff'])) {
			$nprice["DBTaxRulesBill"][$id]["title"]=$item["calc_name"];
			$nprice["DBTaxRulesBill"][$id]["value"]=$cdisp->priceDisplay($this->cart->pricesUnformatted[$id.'Diff']);
		    }
		}
		foreach($cartData["taxRulesBill"] as $id=>$item) {
		    if(isset($this->cart->pricesUnformatted[$id.'Diff'])) {
			$nprice["taxRulesBill"][$id]["title"]=$item["calc_name"];
			$nprice["taxRulesBill"][$id]["value"]=$cdisp->priceDisplay($this->cart->pricesUnformatted[$id.'Diff']);
		    }
		}
		foreach($cartData["DATaxRulesBill"] as $id=>$item) {
		    if(isset($this->cart->pricesUnformatted[$id.'Diff'])) {
			$nprice["DATaxRulesBill"][$id]["title"]=$item["calc_name"];
			$nprice["DATaxRulesBill"][$id]["value"]=$cdisp->priceDisplay($this->cart->pricesUnformatted[$id.'Diff']);
		    }
		}
		...
		foreach($price as $id=>$value) {
			...
			if(in_array($id,array('salesPriceDBT','taxRulesBill','DATaxRulesBill','DBTaxRulesBill'))) {
			    continue;
			}
	...
	function updateProduct() {
		...
		if(($err=$this->cart->getError())!=null) {
			return $err;
		}
		return true;
	}
