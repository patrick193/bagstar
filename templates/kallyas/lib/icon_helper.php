...
class JHtmlIcon
{
	static function create($category, $params)
	{
		...
		$output = '<span title="'.JText::_('COM_CONTENT_CREATE_ARTICLE').'">'.$button.'</span>';
		return $output;
	}
	...
	static function edit($article, $params, $attribs = array())
	{
		...
		if (property_exists($article, 'checked_out') && property_exists($article, 'checked_out_time') && $article->checked_out > 0 && $article->checked_out != $user->get('id')) {
			...
			return '<span title="'.htmlspecialchars($tooltip, ENT_COMPAT, 'UTF-8').'">'.$button.'</span>';
		}
		...
		$text = '<span class="icon-edit icon-white"></span>';
		...
		$button = JHtml::_('link', JRoute::_($url), $text, array("data-rel"=>"tooltip", "title"=> JText::_('COM_CONTENT_EDIT_ITEM').' :: '.$overlib ));

		//$output = '<span title="'.JText::_('COM_CONTENT_EDIT_ITEM').' :: '.$overlib.'">'.$button.'</span>';
		$output = $button;
		//$text = '<span class="icon-edit icon-white"></span>';
		return $output;
	}
	...
	static function print_popup($article, $params, $attribs = array())
	{
		...
		if ($params->get('show_icons')) {
			...
			$text = '<span class="icon-print icon-white"></span>';
		} else {
