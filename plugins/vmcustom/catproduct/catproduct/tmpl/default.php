	...
	print_r($price_per_unit);
	  ...
	  */
	  ...
	  if (empty($product['child']['catproduct_inline_row'])) {		
		// callforprice url
		$callforprice = true;
	  $callforprice_url = 'index.php?option=com_virtuemart&view=productdetails&task=askquestion&tmpl=component&virtuemart_product_id='.$product['child']['virtuemart_product_id'];
		...
		if ($parametri["show_basePrice"] == 1) {
			if ($product['prices']['basePrice'] > 0 or VmConfig::get ('askprice', 0)) $callforprice = false;
		...
		if ($parametri["show_basePriceWithTax"] == 1) {
			if ($product['prices']['basePriceWithTax'] > 0 or VmConfig::get ('askprice', 0)) $callforprice = false;
		...
		if ($parametri["show_priceWithoutTax"] == 1) {
			if ($product['prices']['priceWithoutTax'] > 0 or VmConfig::get ('askprice', 0)) $callforprice = false;
		...
		if ($parametri["show_salesPrice"] == 1) {
			if ($product['prices']['salesPrice'] > 0 or VmConfig::get ('askprice', 0)) $callforprice = false;
		...
		if ($check_stock == 1 && $product['child']['product_in_stock'] > 0 || $check_stock == 0){
			...
			if (!VmConfig::get('use_as_catalog', 0)  ) {
				...
				<span class="quantity-box"><input class="quantity-input js-recalculate" size="2" id="quantity_'.$i.'" name="quantity[]" value="1" type="text" onblur=\'changeQuantity('.$i.', "input", '.$min_order_level.', '.$max_order_level.', '.$product_box .')\'></span>
				...
				echo '<td class="cell_quantity">
				<a class="ask-a-question bold" href="'.$callforprice_url.'" rel="nofollow" >'.JText::_ ('COM_VIRTUEMART_PRODUCT_ASKPRICE').'</a>
				</td>';
				}
			}
			// sum weight
...
<?php
	...
	if ($version->RELEASE <> '1.5') {
		$document->addScript(JURI::root(true). "/plugins/vmcustom/catproduct/catproduct/js/catproduct.js");
