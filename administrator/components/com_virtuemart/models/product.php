...
$where[]=$g['where'];
$joinedTables.=$g['join'];
$_SESSION['filter_where'] = $g['where'];
$_SESSION['filter_join'] = $g['join'];
} else {
$_SESSION['filter_where'] = null;
$_SESSION['filter_join'] = null;
}
}
...
// use lang table only TODO Look if this not cause errors
	function getProductChilds ($product_id) {
        ...
        $db->setQuery('SELECT * FROM `#__virtuemart_products` as p LEFT JOIN `#__virtuemart_products_' . VMLANG . '` as pl USING (`virtuemart_product_id`) LEFT JOIN `#__virtuemart_product_medias` USING (`virtuemart_product_id`) LEFT JOIN `#__virtuemart_medias` USING (`virtuemart_media_id`) LEFT JOIN `#__virtuemart_product_prices` USING (`virtuemart_product_id`) WHERE `product_parent_id` = ' . (int) $product_id);
