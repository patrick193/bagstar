...
<?php
	...
	public function displayCatTree($cat){
		...
		for($i=0; $i<$cat_count; $i++){
			...
			<div class="cat-content<?php echo ($cat[$i]->published==0)?' grayed':'';?>">
				<?php echo $cat[$i]->name; ?>
			</div>
	...
	public function displayPTTreeMenu($pt){
		...
		for ($i=0, $j=count($pt); $i<$j; $i++){ ?>
			<tr>
				<td onmouseover="pttreehighlight(this,1);" onmouseout="pttreehighlight(this,1);" class="pt-td">
...
?>
		...
		for($i=0; $i<$count_rows; $i++) {
			if($orderby=='cat' && !$ppid && $current_category!=$rows[$i]->cid){
				$current_category=$rows[$i]->cid;
			...
			<tr id="<?php echo $currow ?>-tr" class="fs-row" data-row="<?php echo $currow ?>" data-ptid="<?php echo ($ptid)?$ptid:'none' ?>">
				...
				<div class="ui-namecell-pid"><?php echo 'id: '.$rows[$i]->pid ?></div>
				<?php //if($ptid): ?>
				<div class="ui-namecell-ptid<?php if(!$ptid)echo ' hid' ?>"><?php echo 'ptid: '.$ptid ?></div>
				<?php //endif ?>
				<?php if($rows[$i]->published==0) echo '<div class="ui-namecell-unpublish">unpublished</div>'; ?>
				<?php if(fsconf::getOption('show_pdesc_button')==1){ ?>
				<button type="button" id="<?php echo $currow ?>-pdesc" class="ui-namecell-pdesc" onclick="showProductDescription(event,<?php echo $rows[$i]->pid ?>)">i</button>
				<?php } ?>
				<?php 
				if($ppid){
					if($ppid==$rows[$i]->pid) echo '<div class="ui-namecell-child" data="'.$rows[$i]->pid.'" data-skip="'.$skip.'">Parent</div>';
				}
				else if($this->product_has_children($rows[$i]->pid)) echo '<div class="ui-namecell-child" data="'.$rows[$i]->pid.'" data-skip="'.$skip.'">Parent</div>';
				?>
				<br clear="all" />
				
				<div class="ui-namecell-name<?php if($rows[$i]->published==0) echo ' grayed' ?>"><?php
				if($q){// && strpos($q,'(')===false && strpos($q,')')===false){
					/*preg_match_all("/$q+/i", $rows[$i]->product_name, $matches);
					if (is_array($matches[0]) && count($matches[0]) >= 1) {
					    foreach ($matches[0] as $match){
					    	echo $match.'-';
					        $haystack=str_replace($match, '<b>'.$match.'</b>', $rows[$i]->product_name);
					    }
					}
					echo $haystack;*/
					...
					//echo str_ireplace($q,"<b>$q</b>",$rows[$i]->product_name);
					
					//echo $rows[$i]->product_name;
				}else{echo $rows[$i]->product_name;} ?></div></td>
				...
				if($ptid){
					...
					self::displayProductParameters($rows[$i]->pid, $ptid, $ptparams, $currow);
		...
		<div id="filter-dialog" class="popwindow hid" style="left:50%;">
			<div id="filter-dialog-inner"><div style="padding:10px 20px;"> </div></div>
...
?>
	...
	public function printProductTypes($pt=null, $row){
		...
		<div id="prodtype-menu-<?php echo $row ?>" class="popmenu hid">
		<?php
		if($pt){
			foreach($pt as $p){
	...
	public function displayProductParameters($pid, $ptid, $ptparams, $row) {
		...
		//$urlptid=JRequest::getVar('ptid','');
		$q="SELECT * FROM `#__vm_product_type_$ptid` WHERE `product_id`=$pid";
		$db->setQuery($q);
		$res=$db->loadAssoc();
		...
		if($params_count) {
			for($i=0; $i<$params_count; $i++){
				...
				echo '<div style="float:left;max-width:150px;margin:4px 6px 8px 6px;"><button id="'.$row.'-'.$ptparams[$i]->parameter_name.'-parambtn" ';
				...
				if($productsValues){
					...
					//echo ' data-btnvalue=""><span class="ui-parameter-value-empty">['. $ptparams[$i]->parameter_label .']</span></button></div>';
				}
	...
	public function printFilterDialog($name, $data){
		...
		?>
		<div class="pf-title"><?php echo '<span style="width:300px;display:inline-block;">'.$name.':</span> <div style="display:inline-block;vertical-align:top;"><span style="color:#3E55A9">'.$data->parameter_label.'</span> (<span style="color:#BF4545">'.$paramname.'</span>)</div>'; ?></div>
		<div class="pf-cont">
		<table cellpadding="0" cellspacing="0" style="margin:5px 2px 15px 0;border-collapse:collapse;"><tr>
		<?php
...
?>
	...
	public function squeeze($string, $characters = 12){
		$len=strlen($string);
		if($len > $characters){
			$string = mb_substr($string, 0, $characters) .'<b class="threedots">..</b>';
		}
		return $string;
	}
