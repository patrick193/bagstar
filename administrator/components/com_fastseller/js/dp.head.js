...
function rmPTInfoDialog(row) {
	var parent=$(row+'-rmpt'),
		target=parent.getLast(),
		currentMargin=target.style.marginRight.toInt();
	...
	bounceEffect = new Fx.Tween(target, {
		duration: 350,
		transition: Fx.Transitions.Expo.easeOut,
		property: 'margin-right'
	});
	
	if (target.hasClass('dialog-visible')) {
		bounceEffect.start(currentMargin, -120);
	} else {
		bounceEffect.start(currentMargin, 0);
	}
...
function setpage(el,skip){
	if (el.className != 'pager-available') return;
...
function setshowonpage(el){
	simplePopup.activeButton.getFirst().innerHTML=el.innerHTML;
	simplePopup.removemenu();
	simplePopup.removeBoundEvent();
	
	var form=document.searchForm;
	form.showonpage.value=el.innerHTML;
	form.skip.value='';
	var hash=el.href;
